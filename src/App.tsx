...
function App() {
  ...
  function handleClick(event: React.MouseEvent) { 
    ...
    const newEvent:clickEvent = {id:crypto.randomUUID(), time: Date.now(), target:event.target}
  ...
  function replayNextInput(index:number){ 
    ...
    if(index < interceptedEvents.length-1){
      ...
      const delay = interceptedEvents[index +1 ].time - interceptedEvents[index].time;
      setTimeout(() => replayNextInput(index+1), delay);
    }
  }
  function onCellValueChanged(event: CellKeyDownEvent){
      const keypress: keydownEvent = {id: crypto.randomUUID(), time: Date.now(), key: event.event.key, target: event.event.target, event: event} 
  ...
  return (
    < div onMouseMove={(e) => handleMouseMove(e)} style={{height: '100vh'}}>
      ...
      <Sheet onCellValueChangedCallback={onCellValueChanged} gridRef={gridRef}/>
      <VideoControl/> 
    </div>
